...
5. [변경 이력 (Changelog) — 쿼리 업데이트 기록](#5-변경-이력-changelog--쿼리-업데이트-기록)
...
| 버킷 | 의미 | 사용자에게 주는 가치 |
|---|---|---|
| **충동** | 검색 없이 담았고 이후 관심도 없음 (또는 배너/SNS 유입) | "정리해도 되는 상품" |
| **클렌징_시즌** | 지금 계절과 안 맞는 시즌 상품 | "다음 시즌에" |
| **클렌징_니즈해결** | 담은 뒤 같은 카테고리를 이미 구매 | "이미 해결됨" |
| **고민** | 최근 45일 내 클릭·재검색 등 관심 신호가 있음 | "조금 더 고민 중" |
...
3) 지금 계절과 안 맞는 시즌 상품이다
        ├ 그래도 클릭한 적 있으면               → 고민
        └ 아니면                               → 클렌징_시즌
4) 최근 45일 내 클릭·카테고리 검색 + 배너/SNS 유입이 아니다 → 고민
5) 위 어디에도 안 걸린다                        → 충동
```
...
> - 우선순위(보관>니즈해결>시즌>고민>충동)는 기획서 버킷 정의에 따른다.
> - **45일 Decay** — 클릭 관심신호(`clk`)는 `searched_at >= DATE_SUB(NOW(), INTERVAL 45 DAY)`
>   인 최근 클릭만 유효하다. 오래된 클릭 이력이 지금까지 '고민'으로 잡히는 것을 막는다.
> - **배너/SNS 유입 제외** — 관심신호가 있어도 `referrer IN ('event_banner','sns_ad')` 이면
>   고민이 아닌 **충동**으로 본다(이벤트/SNS로 유입된 검색은 능동적 관심으로 보지 않음).
> - 버킷 판별은 **별도 정답 라벨 없이** 조회 시점의 이 CASE 결과 자체가 정답이다(4.2 참조).
...
**무엇을 위한 기능? (파일럿과의 차이)**
파일럿(1.2)에서는 별도 정답 라벨 없이 조회할 때마다 CTE로 **즉시 판정(on-the-fly)** 한다.
판별 로직 자체가 정답이므로 조회 결과를 그대로 화면에 쓴다. 하지만 정식 서비스에서는
...
구분자를 운영 컬럼 `bucket` 에 row별로 적재**해 두고, 조회 측은 **컬럼을 읽기만** 한다.
즉 *판정(쓰기)* 과 *조회(읽기)* 를 분리한다. (파일럿에는 정답 라벨·적재 컬럼이 없다.)

```
[파일럿]  조회 시점마다 CTE 4개 집계 → CASE 즉시 판정 → 화면 (별도 라벨 없음)
[정식]    (배치/트리거) CTE 4개 집계 → CASE 판정 → bucket 컬럼 적재
          조회 측은 WHERE bucket IS NOT NULL 로 컬럼만 읽음
```
...
WHERE  c.days_in_cart >= :stale_days               -- 오래 방치된 것만
  AND  c.bucket IS NULL;                            -- 아직 미판별 row만 (재실행 안전)
```
...
SELECT  :cart_id, :user_id, :product_id, :added_at, :days_in_cart,
        ...
        CASE
            ...
            WHEN (clk.product_id IS NOT NULL OR kw_cat.category_id IS NOT NULL)
                 AND :referrer NOT IN ('event_banner', 'sns_ad')            THEN '고민'
            ELSE '충동'
        END
FROM    products   p
JOIN    categories cat ON cat.category_id = p.category_id
...
ON DUPLICATE KEY UPDATE
        bucket = VALUES(bucket);   -- 재담기 시 재판별
```
...
SELECT  c.cart_id, c.user_id, c.product_id, c.added_at, c.days_in_cart,
        c.referrer, c.quantity, c.bucket AS bucket,
        p.product_name, p.brand, p.category_id, p.key_ingredients,
        p.suitable_season, p.texture, p.volume_ml, p.volume_unit, p.price,
        cat.category_name, cat.avg_lifespan_days
FROM    cart_items c
...
WHERE   c.user_id = :user_id
  ...
  AND   c.bucket IS NOT NULL          -- 이미 판별 완료된 것만
ORDER BY c.days_in_cart DESC;
```
...
> - **재실행 안전성** — 배치는 `bucket IS NULL` 조건으로 미판별 row만 갱신한다.
>   구매·검색 신호가 바뀌어 **전체 재분류**가 필요하면 `bucket = NULL` 로 리셋 후
>   재실행하거나, `WHERE` 의 NULL 조건을 빼고 전 row를 덮어쓴다.
> - **컬럼 운영** — 정식 전환 시 운영 컬럼명을 `bucket` 으로 두고 `bucket_decided_at`(판별
>   시각) 을 함께 적재하면 "언제 기준의 분류인지" 추적과 캐시 무효화가 쉬워진다.
>   (파일럿은 별도 정답 라벨 없이 조회 시점 CASE 결과 자체가 정답이므로 적재 컬럼이 없다.)
...
### 1.3 버킷 분포용 유저 목록 — `SQL_ALL_STALE_USERS`

**무엇을 위한 기능?**
버킷 분포를 집계(`bucket_distribution()`)할 때, 방치 상품을 가진 **모든 유저**를 한 번에 뽑아
1.2 분류 결과를 버킷별로 집계하는 데 쓴다(별도 정답 라벨 없이 CASE 결과 자체를 집계).
추천 화면 자체에는 쓰이지 않는다.
...
> 기획서 버킷 정의(충동/고민 구분 등), ③ pymysql/MySQL 엔진 제약 때문에 불가피했던 것이다.
> 버킷 판별은 별도 정답 라벨 없이 **조회 시점 SQL CASE 판별 결과 자체가 정답**인 구조다.
...
### 4.2 기획서 버킷 정의를 반영한 의도적 차이

문서 I 2.6의 통합 CASE와 **판정 규칙·우선순위가 다르다**. 버킷 판별은 별도 정답 라벨 없이
조회 시점 CASE 결과 자체가 정답이며, 아래는 기획서 버킷 정의(충동/고민 구분 등)에 맞춘 결정이다.

| 분기 | 문서 I | 구현(문서 II) | 사유 |
|---|---|---|---|
| 우선순위 | 니즈해결 > 시즌 > 보관 > 고민 > 충동 | **보관 > 니즈해결 > 시즌 > 고민 > 충동** | 보관(재구매)을 최우선 확정 |
| 보관(반복구매) | `purchased_at < added_at` (담기 전만) | **시점 무관** `COUNT(*) >= 2` | 담은 후 구매도 반복구매 신호로 인정 |
| 시즌 + 클릭 | 무조건 `클렌징_시즌` | 클릭(`product_clicked`) 있으면 **`고민`** | 관심신호가 있으면 보관 가치로 재분류 |
| 관심신호(클릭) 유효기간 | 명시 없음 | **최근 45일 내**(`searched_at >= NOW()-45d`) | 오래된 클릭이 계속 고민으로 잡히는 것 방지(45일 Decay) |
| 고민 제외 조건 | 없음 | **`referrer NOT IN ('event_banner','sns_ad')`** | 이벤트/SNS 유입 + 검색은 능동 관심으로 보지 않고 충동 처리 |
| 니즈해결 상품 | `product_id != cart.product_id` (다른 상품) | 같은 상품 제외 안 함 | 보관 우선순위가 위에서 해소하므로 결과 동일, 식 단순화 |

> 이 규칙들은 기획서 v0.3 버킷 정의(충동 = 이벤트/SNS 유입 + 검색 이력 없음, 고민 = 최근 45일
> 관심신호)를 반영한 것이다. 초기 파일럿은 `expected_bucket` 정답 라벨로 일치율을 쟀으나,
> 현재는 라벨을 제거하고 **판별 로직 자체를 정답**으로 삼는다(1.3 버킷 분포로 결과 검증).
...
## 5. 변경 이력 (Changelog) — 쿼리 업데이트 기록

> 발표/리뷰 추적용. 쿼리·구조 변경은 반드시 여기에 누적 기록한다.

### 2026-06-12 — 정답 라벨 제거 + 버킷 판별 로직 강화

**배경.** 사전 라벨(`expected_bucket`)과 실시간 SQL 판별 결과 사이에 약 4% 불일치가 있어,
별도 정답 라벨을 두는 구조를 폐기했다. **버킷 판별은 조회 시점에 SQL CASE WHEN 으로 실시간
계산하며, 별도 라벨 없이 판별 로직 자체가 정답인 구조**로 전환했다.

| # | 대상 쿼리 / 구성요소 | 변경 내용 | 사유 |
|---|---|---|---|
| ① | `SQL_STALE_CART` — `clk` CTE | `WHERE` 절에 `searched_at >= DATE_SUB(NOW(), INTERVAL 45 DAY)` 추가 | **45일 Decay**: 오래된 클릭 이력이 계속 '고민'으로 잡히는 것 방지 |
| ② | `SQL_STALE_CART` — 고민 분기 (CASE 4단계) | `(clk OR kw_cat)` → `(clk OR kw_cat) AND c.referrer NOT IN ('event_banner','sns_ad')` | **배너/SNS 유입 제외**: 이벤트/SNS 유입 + 검색은 능동 관심으로 보지 않고 충동 처리 |
| ③ | `SQL_STALE_CART` — `SELECT` | `c.expected_bucket` 컬럼 제거 | 정답 라벨 폐기 (실시간 판별 결과 자체가 정답) |
| ④ | `cart_items` 스키마 | `expected_bucket`·`expected_reason` 컬럼 제거 (CSV·DB·`upload_to_mysql.py` 오버라이드) | 정답 라벨 폐기 |
| ⑤ | `SQL_BUCKET_PERSIST` / `SQL_BUCKET_INSERT_ONE` / `SQL_STALE_CART_READ` (1.2-P 정식) | 적재·조회 컬럼명 `expected_bucket` → `bucket`, ①·② 로직 동일 반영 | 운영 컬럼 의미 정합 + 파일럿 로직과 100% 동일 유지 |
| ⑥ | 검증 기능 | `validate_all()`(일치율) → `bucket_distribution()`(버킷 분포 집계). API `/api/validate` → `/api/distribution`. 프론트 "🔍 분류 검증" → "📊 버킷 분포" 뷰 | 정답 라벨이 없으므로 일치율 대신 분포로 결과 검증 |

**검증.** 재업로드 후 `bucket_distribution()` 실행 결과: 방치 유저 100명 · 상품 871개
(클렌징 456 / 보관·고민 415), 버킷 분포 = 클렌징_시즌 218 · 고민 214 · 보관 201 · 충동 164 ·
클렌징_니즈해결 74.
