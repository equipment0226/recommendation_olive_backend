...
결정트리(검증 일치율 96%):
  1) 동일상품 반복구매 N회+ → catmatch 면 '보관+고민' else '보관'
...
BUCKET_META = {
    "클렌징_충동":   {"group": "cleansing", "type": "충동", "default_checked": True},
    ...
    "보관+고민":     {"group": "keep",      "type": "보관", "default_checked": False},
}
...
SELECT  c.cart_id, c.user_id, c.product_id, c.added_at, c.days_in_cart,
        c.referrer, c.quantity, c.expected_bucket, c.expected_reason,
...
def _build_reason(bucket: str, row: dict) -> str:
    ...
    return {
        "클렌징_충동": "검색 없이 담았고 이후 관심 신호가 없어요. "
        ...
        "보관": "동일 상품을 2회 이상 반복 구매하신 이력이 있어요. "
              "소진 시점에 맞춰 재구매를 권해요.",
        "보관+고민": "늘 쓰시던 상품을 반복 구매하면서, 같은 카테고리의 다른 상품도 "
                  "함께 검색 중이세요.",
    }.get(bucket, row.get("expected_reason", ""))
...
def classify_cart_item(row, repeat_counts, purchases, searches) -> str:
    ...
    # 우선순위 결정트리
    if repeat2:
        return "보관+고민" if catmatch else "보관"
    if samecat_after:
        return "클렌징_니즈해결"
    if season_mis:
